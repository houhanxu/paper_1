...
%Array codes are error-correcting codes that have been widely deployed in
%distributed storage systems (e.g., Redundant Arrays of Inexpensive Disks
%(RAID) \cite{patterson1989introduction}).
...
%We refer to a disk as a column or a storage node interchangeably, and an
%entry in the array as a bit.  When a node fails, the corresponding column of
%the array code is considered to be an \emph{erasure}.
...
%In the framework in \cite{dimakis2010network}, a data file of size $B$
%symbols is encoded and distributed to $n$ storage nodes, with each node
%storing $L$ symbols such that the file can be decoded from any $k$ of them.
%Furthermore, upon the failure of a storage node, a new node replaces the
%failed node by downloading $\beta$ symbols from each of $d$ surviving nodes,
%and the repair bandwidth is $d\beta$.
\begin{equation}
\frac{(k+1)L}{2},
